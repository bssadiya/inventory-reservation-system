...
import { Database, Clock, ShieldCheck, XCircle, RefreshCw, AlertCircle } from "lucide-react";
...
export default function ReservationsMonitor({ 
  ...
  const getStatusBadge = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-gray-150 border border-gray-255 text-gray-600 text-xs font-semibold uppercase font-mono tracking-wider">
            {status}
          </span>
        );
    }
      ...
      {reservations.length === 0 ? (
        ...
        <div className="overflow-x-auto -mx-6 sm:mx-0">
          <table className="w-full text-left text-sm text-gray-500 whitespace-nowrap">
            <thead className="bg-gray-50 text-gray-400 font-bold uppercase text-[10px] tracking-wider border-b border-gray-100 font-mono">
              <tr>
                <th className="px-6 py-4">ID</th>
                <th className="px-6 py-4">Product Details</th>
                <th className="px-6 py-4">Warehouse Location</th>
                <th className="px-6 py-4 text-center sm:text-left">Qty</th>
